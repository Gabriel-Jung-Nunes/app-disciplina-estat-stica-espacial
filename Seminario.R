  ...
  observeEvent(input$estadoSelecionado, {
    ...
    output$mapa_woow <- renderLeaflet({
      ...
      faz_tabela <- function(municipio) {
        ...
        dados_tabela <- dados_merged |> 
          filter(NM_MUN == municipio) |> 
          select(Bovino, Equino, Suinos, Galinaceos, AREA_KM2, Pop_residente) |> 
          st_drop_geometry()
        
        # Valores específicos para cálculos
        ...
        tabela <- paste0("<b><span style='font-size: 13px;'>", municipio, "</span></b><br><br>",
                         "<span style='font-size: 11px;'>", tabela, "</span>")
